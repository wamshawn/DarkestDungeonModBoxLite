import {App, Badge, Card, Flex, FloatButton, Image, Spin, Tag} from "antd";
import {RetweetOutlined, ClockCircleOutlined} from '@ant-design/icons';
...
import {ListModules} from "../../../wailsjs/go/box/Box";
...
const Index = () => {
    ...
    useAsyncEffect(async () => {
        ...
        state.loading = true;
        const r = await rpc<void, Array<box.WorkshopModule>>(ListModules)
        state.loading = false;
        if (r.failed()) {
            const errs = r.cause()
    ...
    const syncModule = async (id: string) => {
        const target = state.syncing.find((v) => v.id === id)
        if (target) {
            target.sync = true
            setTimeout(()=> {
                target.sync = false
    ...
    return (
        <Flex>
            ...
            <FloatButton icon={<RetweetOutlined/>} type="primary" tooltip={<div>同步至模组管理器</div>}/>
        </Flex>
